...
async function generatePDF() {
    try {
        // Read the HTML file
        const htmlPath = './templates/Wayne_Davis_Compensation_Agreement.html';
        const logoPath = './assets/lendwise-logo.png';
        const outputPath = './PDFs/Wayne_Davis_Compensation_Agreement.pdf';
    ...
    <p><strong>Personal Materials:</strong> Loan Officer retains ownership of marketing materials created entirely on their own time using their own resources and not incorporating any Lendwise confidential information, provided such materials do not use the Lendwise name, logo, or branding.</p>`
        ...
        const tempHtmlPath = './Wayne_Davis_Contract_with_Logo.html';
        ...
        // Convert relative path to absolute for browser
        const absoluteTempPath = path.resolve(tempHtmlPath);
        await page.goto(`file://${absoluteTempPath}`, { waitUntil: 'networkidle' });
